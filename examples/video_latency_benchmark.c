/* examples/video_latency_benchmark.c */
...
#define FRAME_SIZE 8000 /* 8KB general data payload */
...
typedef struct {
  ...
  bool subscribed;
  bool use_reliable;
  frame_metrics_t frames[MAX_FRAMES];
  int frames_received;
...
static void on_server_event(void *user_data, const transport_event_t *event) {
  ...
  case TRANSPORT_EVENT_SUBSCRIBE:
    if (strcmp(event->track_id.name, "video_benchmark") == 0) {
...
static void on_client_event(void *user_data, const transport_event_t *event) {
  ...
  case TRANSPORT_EVENT_OBJECT:
    if (strcmp(event->track_id.name, "video_benchmark") == 0) {
      ...
      if (fid < MAX_FRAMES) {
        state->frames[fid].recv_time = get_time_ms();
        state->frames[fid].arrived = true;
...
int main(int argc, char **argv) {
  ...
  printf("starting video latency benchmark (%s mode)...\n",
  ...
  strcpy(track.name, "video_benchmark");
  ...
  printf("handshake established. streaming 200 frames at 60fps...\n");

  uint8_t *frame_payload = malloc(FRAME_SIZE);
  memset(frame_payload, 0xAA, FRAME_SIZE);
  ...
  for (int f = 0; f < MAX_FRAMES; f++) {
    ...
    client_state.frames[f].send_time = get_time_ms();
    moq_object_t obj = {
        .track_id = track,
        .group_id = 0,
        .object_id = f,
        .data = frame_payload,
        .size = FRAME_SIZE,
        .is_keyframe = (f % 30 == 0) /* keyframe every 30 frames */
    };
  ...
  for (int f = 0; f < MAX_FRAMES; f++) {
    if (client_state.frames[f].arrived) {
      double lat =
          client_state.frames[f].recv_time - client_state.frames[f].send_time;
      total_latency += lat;
      if (lat > max_latency) {
        max_latency = lat;
      }
      ...
      if (last_recv_time > 0) {
        double gap = client_state.frames[f].recv_time - last_recv_time;
        if (gap > 33.3) { /* missed more than one frame render interval */
          freezes++;
        }
      }
      last_recv_time = client_state.frames[f].recv_time;
    }
  }
  ...
  printf("received: %d\n", samples);
  printf("loss_pct: %.1f%%\n",
         ((double)(MAX_FRAMES - samples) / MAX_FRAMES) * 100.0);
